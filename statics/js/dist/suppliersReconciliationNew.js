define(["jquery", "print"], function(a) {
	function b() {
		o.cssCheckbox(), "true" == k.showDetail && (o.find("label").addClass("checked"), p[0].checked = !0), k.beginDate && k.endDate && i("div.grid-subtitle").text("日期: " + k.beginDate + "至" + k.endDate), i("#filter-fromDate").val(k.beginDate), i("#filter-toDate").val(k.endDate), i("#customer input").val(k.customerName), Business.supplierCombo(i("#customer"), {
			defaultSelected: 0,
			addOptions: {
				text: r,
				value: 0
		...
		i("#filter-submit").on("click", function(c) {
			...
			return g.getTime() > j.getTime() ? void parent.Public.tips({
				...
				supplierName: m,
				showDetail: n
			}, i("div.grid-subtitle").html("<p>供应商：" + m + "</p><p>日期: " + e + " 至 " + f + "</p>"), void h(o))
		}), i("#filter-reset").on("click", function(a) {
			a.preventDefault(), i("#filter-fromDate").val(""), i("#filter-toDate").val(""), i("#customer input").val("")
		})
	}
	function c() {
		i("#btn-print").click(function(a) {
			...
			if (i(this).data("hasInstance")) this.customerDialog.show().zindex();
				...
				var a = i("#customer").prev().text().slice(0, -1),
					b = "选择" + a;
				if ("供应商" === a || "采购单位" === a) var c = "url:../settings/select_customer?type=10&multiselect=false";
	...
	var i = a("jquery"),
		...
		l = "../report/supplierBalance_exporter?action=exporter",
		m = "../report/supplierBalance_detail?action=detail",
		n = i("#customer"),
		o = i("#match"),
