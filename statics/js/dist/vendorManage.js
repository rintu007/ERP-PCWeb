	...
	delete b.firstLink, Public.ajaxPost("../basedata/contact/" + ("add" == oper ? "add" : "update")+"?type=10", b, function(d) {
		if (200 == d.status) {
			if (parent.parent.Public.tips({
				...
				var e = d.data.id;
				d = b, d.id = e, d.customerType = d.cCategoryName, d.contacter = c.linkName || "", 
				d.mobile = c.linkMobile || "", 
...
function initField() {
	if ($("#note").placeholder(), "edit" == oper) {
		if ($("#name").val(rowData.name), $("#category").data("defItem", ["id", rowData.cCategory]), rowData.beginDate) {
