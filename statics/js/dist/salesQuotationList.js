var queryConditions = {
	...
	THISPAGE = {
		init: function() {
			SYSTEM.isAdmin !== !1 || SYSTEM.rights.AMOUNT_OUTAMOUNT || (hiddenAmount = !0), this.mod_PageConfig = Public.mod_PageConfig.init("salesQuotationList"), this.initDom(), this.loadGrid(), this.addEvent()
		},
		initDom: function() {
		...
		loadGrid: function() {
			...
			function b(a, b, c) {
				return 159001 === a ? "报价" : (d.markRow.push(c.id), "退货")
			}
		...
		addEvent: function() {
			...
			if ($(".grid-wrap").on("click", ".ui-icon-pencil", function(a) {
				...
				parent.tab.addTabItem({
					...
					text: "销售报价单",
					url: "../scm/invSq?action=editSq&id=" + b + "&flag=list" + d
				});
				$("#grid").jqGrid("getDataIDs");
				parent.cacheList.salesQuotationId = $("#grid").jqGrid("getDataIDs")
			}), $(".grid-wrap").on("click", ".ui-icon-trash", function(a) {
				if (a.preventDefault(), Business.verifyRight("SQ_DELETE")) {
					var b = $(this).parent().data("id");
					$.dialog.confirm("您确定要删除该销售记录吗？", function() {
						Public.ajaxGet("../scm/invSq/delete?action=delete", {
							id: b
						}, function(a) {
				...
				a.preventDefault(), Business.verifyRight("SQ_导入") && parent.$.dialog({
					width: 560,
					height: 300,
					title: "批量导入",
					content: "url:../sales/import?type=sq",
					lock: !0
				})
			}), $(".wrapper").on("click", "#export", function(a) {
				if (!Business.verifyRight("SQ_EXPORT")) return void a.preventDefault();
				var b = $("#grid").jqGrid("getGridParam", "selarrrow"),
					c = b.join(),
					d = c ? "&id=" + c : "";
				for (var e in queryConditions) queryConditions[e] && (d += "&" + e + "=" + queryConditions[e]);
				var f = "../scm/invSq/exportInvSq?action=exportInvSq" + d;
				$(this).attr("href", f)
			}), billRequiredCheck) {
				{
				...
				$(".wrapper").on("click", "#audit", function(a) {
					...
					var b = $("#grid").jqGrid("getGridParam", "selarrrow"),
						c = b.join();
					return c ? void Public.ajaxPost("../scm/invSq/batchCheckInvSq?action=batchCheckInvSq", {
						id: c
					}, function(a) {
					...
					var b = $("#grid").jqGrid("getGridParam", "selarrrow"),
						c = b.join();
					return c ? void Public.ajaxPost("../scm/invSq/rsBatchCheckInvSq?action=rsBatchCheckInvSq", {
						id: c
					}, function(a) {
						parent.Public.tips(200 === a.status ? {
							content: a.msg
			...
			$("#search").click(function() {
				...
				queryConditions.matchCon = a.$_matchCon.val(), queryConditions.beginDate = a.$_beginDate.val(), queryConditions.endDate = a.$_endDate.val(), $.dialog({
					...
					resize: !1,
					content: "url:../sales/sales_search?type=sq",
					data: queryConditions
				})
			}), $("#refresh").click(function() {
				THISPAGE.reloadData(queryConditions)
			}), $("#add").click(function(a) {
				a.preventDefault(), Business.verifyRight("SQ_ADD") && parent.tab.addTabItem({
					tabid: "sales-salesQuotation",
					text: "销售报价单",
					url: "../scm/invSq?action=initSq"
				})
