var curRow, curCol, loading, SYSTEM = system = parent.SYSTEM,
	...
	THISPAGE = {
		init: function(a) {
			this.mod_PageConfig = Public.mod_PageConfig.init("salesQuotation"), 
			SYSTEM.isAdmin !== !1 || SYSTEM.rights.AMOUNT_OUTAMOUNT || (hiddenAmount = !0, $("#amountArea").hide()), 
			this.initCombo(), 
			this.initDom(a), 
			this.loadGrid(a), 
		...
		initDom: function(a) {
			...
			this.customerCombo = Business.billCustomerCombo($("#customer"), {
				...
				callback: {
					onChange: function(a) {
						a ? ($("#customer").data("contactInfo", a), b.setSaleByContact(a)) 
								: $("#customer").removeData("contactInfo")
					}
				}
			}),
			 this.currencyCombo = Business.currencyCombo($("#currency"), {
				...
				callback: {
					onChange: function(a) {
						// this.currencyCombo.selectByValue(a.currency);
					}
				}
			}),				
			...
			this.classBox = this.$_classes.cssRadio({
				callback: function(a) {
					b.$_paymentTxt.text("159001" == a.find("input").val() ? "本次收款:" : "本次退款:")
				}
			}), this.classBox.setValue(159001 == a.transType ? 0 : 1), a.description 
			&& this.$_note.val(a.description), 
			this.$_discountRate.val(a.disRate),
			this.$_deduction.val(a.disAmount),
			this.$_discount.val(a.amount),
			this.$_payment.val(a.rpAmount),
			this.$_arrears.val(a.arrears), requiredMoney && ($("#accountWrap").show(), this.accountCombo = Business.accountCombo($("#account"), {
				width: 112,
				height: 300,
				emptyOptions: !0,
				addOptions: {
			...
			var d = '<a id="savaAndAdd" class="ui-btn ui-btn-sp">保存并新增</a><a id="save" class="ui-btn">保存</a>',
				e = '<a id="add" class="ui-btn ui-btn-sp">新增</a><a href="../scm/invSq/toPdf?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">打印</a><a id="edit" class="ui-btn">保存</a>',
				f = '<a id="add" class="ui-btn ui-btn-sp">新增</a><a href="../scm/invSq/toPdf?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">打印</a>',
				g = "",
				h = "",
				i = "159002" == originalData.transType ? "生成退货单" : "生成销售报价单",
			...
			this.btn_edit = e, this.btn_audit = g, this.btn_view = f, this.btn_reaudit = h, this.btn_turn = j, a.id > 0 ? (this.$_number.text(a.billNo),
			 this.$_date.val(a.date),
					this.$_deliveryDate.val(a.deliveryDate), 
					this.$_totalArrears.val(a.totalArrears),
					 this.$_accountInfo.data("accountInfo", a.accounts), -1 === a.accId && (this.$_accountInfo.show(), 
					 b.$_payment.attr("disabled", "disabled").addClass("ui-input-dis")), $("#grid").jqGrid("footerData", "set", {
				qty: a.totalQty,
				amount: a.totalAmount
			}), "list" !== urlParam.flag && (k = ""),
			 "edit" === a.status ? this.$_toolBottom.html("<span id=groupBtn>" + e + g + "</span>" + k) : a.checked ? ($("#mark").addClass("has-audit"), 
			 this.$_toolBottom.html('<span id="groupBtn">' + j + f + h + "</span>" + k)) : this.$_toolBottom.html('<span id="groupBtn">' + f + "</span>" + k), 
			 this.idList = parent.cacheList.salesQuotationId || [], 
			 this.idPostion = $.inArray(String(a.id), this.idList),
			  this.idLength = this.idList.length, 0 === this.idPostion && $("#prev").addClass("ui-btn-prev-dis"), 
			 this.idPostion === this.idLength - 1 && $("#next").addClass("ui-btn-next-dis"), 
			 this.$_userName.html(a.userName), 
			 this.$_modifyTime.html(a.modifyTime), this.$_checkName.html(a.checkName)) : (this.$_toolBottom.html(billRequiredCheck ? "<span id=groupBtn>" + d + g + "</span>" : '<span id="groupBtn">' + d + "</span>"), this.$_userName.html(system.realName || ""), 
			 this.$_modifyTime.parent().hide(), 
			 this.$_checkName.parent().hide()), disEditable && (THISPAGE.disableEdit(), this.$_toolBottom.hide())
		},
		loadGrid: function(a) {
			function b(a) {
				if (taxRequiredCheck) {
		...
		setSaleByContact: function(a) {
			var b = this;
			b.salesCombo && Public.ajaxGet("../scm/invSq/findNearSqEmp?action=findNearSqEmp", {
				buid: a.id
			}, function(a) {
		...
		disableEdit: function() {
			this.customerCombo.disable(), this.salesCombo.disable(), this.$_date.attr("disabled", "disabled").addClass("ui-input-dis"), 
			this.$_deliveryDate.attr("disabled", "disabled").addClass("ui-input-dis"), 
			this.$_note.attr("disabled", "disabled").addClass("ui-input-dis"),
			this.$_discountRate.attr("disabled", "disabled").addClass("ui-input-dis"), this.$_deduction.attr("disabled", "disabled").addClass("ui-input-dis"), this.$_payment.attr("disabled", "disabled").addClass("ui-input-dis"), this.accountCombo.disable(), $("#grid").jqGrid("setGridParam", {
				cellEdit: !1
			}), this.editable = !1
		},
		enableEdit: function() {
			disEditable || (this.salesCombo.enable(), this.customerCombo.enable(), this.$_date.removeAttr("disabled").removeClass("ui-input-dis"), 
					this.$_deliveryDate.removeAttr("disabled").removeClass("ui-input-dis"), 
					this.$_note.removeAttr("disabled").removeClass("ui-input-dis"),
					this.$_discountRate.removeAttr("disabled").removeClass("ui-input-dis"), this.$_deduction.removeAttr("disabled").removeClass("ui-input-dis"), this.$_payment.removeAttr("disabled").removeClass("ui-input-dis"), this.accountCombo.enable(), $("#grid").jqGrid("setGridParam", {
				cellEdit: !0
			}), this.editable = !0)
		},
		...
		addEvent: function() {
			...
			this.$_deliveryDate.bind("keydown", function(a) {
				...
				b.test($(this).val()) || (parent.Public.tips({
					type: 2,
					content: "交货周期格式有误！如：30"
				}), $(this).val(a.dateValue))
			}), this.$_note.enterKey(),
			this.$_discount.enterKey(), this.$_discountRate.enterKey(), $(".grid-wrap").on("click", ".ui-icon-triangle-1-s", function() {
				var a = $(this).siblings(),
					b = a.getCombo();
				setTimeout(function() {
					b.active = !0, b.doQuery()
				}, 10)
			}), Business.billsEvent(a, "SQ"), this.$_deduction.keyup(function() {
				...
				d && ("edit" === originalData.stata && (d.id = originalData.id, d.stata = "edit"), c.ajaxPost("../scm/invSq/add?action=add", {
				...
				var c = $(this),
					d = THISPAGE.getPostData();
				d && c.ajaxPost("../scm/invSq/addNew?action=addNew", {
					postData: JSON.stringify(d)
				}, function(b) {
					if (200 === b.status) {
				...
				if (b.preventDefault(), Business.verifyRight("SQ_UPDATE")) {
					var c = $(this),
						d = THISPAGE.getPostData();
					d && c.ajaxPost("../scm/invSq/updateInvSq?action=updateInvSq", {
						postData: JSON.stringify(d)
					}, function(b) {
				...
				if (b.preventDefault(), Business.verifyRight("SQ_CHECK")) {
					var c = $(this),
						d = THISPAGE.getPostData();
					d && c.ajaxPost("../scm/invSq/checkInvSq?action=checkInvSq", {
						postData: JSON.stringify(d)
					}, function(b) {
						200 === b.status ? (originalData.id = b.data.id, $("#mark").addClass("has-audit"), $("#edit").hide(), a.$_checkName.html(SYSTEM.realName).parent().show(), a.disableEdit(), $("#groupBtn").html(a.btn_turn + a.btn_view + a.btn_reaudit), $("#turn").html("159002" == a.classBox.getValue() ? "生成退货单" : "生成销售单"), parent.Public.tips({
							content: "审核成功！"
						})) : parent.Public.tips({
							type: 1,
							content: b.msg
				...
				if (b.preventDefault(), Business.verifyRight("SQ_UNCHECK")) {
					var c = $(this);
					c.ajaxPost("../scm/invSq/rsBatchCheckInvSq?action=rsBatchCheckInvSq", {
						id: originalData.id
					}, function(b) {
				...
				a.preventDefault(), Business.verifyRight("SQ_ADD") && parent.tab.overrideSelectedTabItem({
					tabid: "sales-salesQuotation",
					text: "销售报价单",
					url: "../scm/invSq/initSq?action=initSq"
				})
			}), $(".wrapper").on("click", "#print", function(a) {
				a.preventDefault(), Business.verifyRight("SQ_PRINT") && Public.print({
					title: "销售报价单列表",
					$grid: $("#grid"),
					pdf: "../scm/invSq/toPdf?action=toPdf",
					billType: 10304,
					filterConditions: {
						id: originalData.id
					}
				...
				if ("159002" == e) var c = "销售退货单",
					d = "sales-salesBack";
				parent.tab.addTabItem({
					tabid: d,
					text: c,
					url: "../scm/invSq?action=initSq&id=" + originalData.id + "&flag=list&turn&transType=" + e
				}), parent.tab.reload(d)
			}), $("#prev").click(function(b) {
				return b.preventDefault(), $(this).hasClass("ui-btn-prev-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有上一张了！"
				}), !1) : (a.idPostion = a.idPostion - 1, 0 === a.idPostion && $(this).addClass("ui-btn-prev-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/invSq/update?action=update", {
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-next-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有下一张了！"
				}), !1) : (a.idPostion = a.idPostion + 1, a.idLength === a.idPostion + 1 && $(this).addClass("ui-btn-next-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/invSq/update?action=update", {
					id: a.idList[a.idPostion]
				}, function(a) {
...
$(function() {
	if (urlParam.id) {
		if (!hasLoaded) {
			var a = $(".bills").hide();
			Public.ajaxGet("../scm/invSq/update?action=update", {
				id: urlParam.id
			}, function(b) {
