		...
		rules: {
			...
			myRemote: function(a, b, c) {
				return c.old.value === a.value || $(a).data("tip") === !1 && a.value.length > 1 ? !0 : $.ajax({
					url: "../basedata/contact/getNextNo?action=getNextNo",
					type: "post",
					data: "skey=" + a.value,
					dataType: "json",
					success: function(b) {
	...
	delete b.firstLink, Public.ajaxPost("../basedata/contact/" + ("add" == oper ? "add" : "update"), b, function(d) {
		if (200 == d.status) {
			if (parent.parent.Public.tips({
				...
				var e = d.data.id;
				d = b, d.id = e, d.customerType = d.cCategoryName, d.contacter = c.linkName || "", d.mobile = c.linkMobile || "", 
				d.telephone = c.linkPhone || "",
				d.fax = c.linkFax || "",
				d.linkTitle = c.linkTitle || "",
				d.linkIm = c.linkIm || "", 
				d.zipcode = c.linkZipcode || "",
				d.deliveryAddress = c.address || "", 
				d.province = c.province || "",
				d.city = c.city || "", d.county = c.county || "", 
				callback(d, oper, window)
			}
...
function initField() {
	if ($("#note").placeholder(), "edit" == oper) {
		if ($("#name").val(rowData.name), 
				$("#category").data("defItem", ["id", rowData.cCategory]), rowData.beginDate) {
		...
		rowData.bank && $("#bank").val(rowData.bank),
		rowData.account && $("#account").val(rowData.account),
		rowData.taxnumber && $("#taxnumber").val(rowData.taxnumber)
	} else $("#date").val(parent.parent.SYSTEM.startDate);
	api.opener.parent.SYSTEM.isAdmin || api.opener.parent.SYSTEM.rights.AMOUNT_OUTAMOUNT || ($("#receiveFunds").closest("li").hide(),
...
function initGrid(a) {
	...
	a.push({}), $grid.jqGrid({
		...
		loadComplete: function(a) {
			if ($grid.setGridHeight($grid.height() > 124 ? "124" : "auto"), $grid.setGridWidth(800), "add" != oper) {
