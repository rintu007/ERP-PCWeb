...
var THISPAGE = {
    ...
    loadGrid: function () {
        ...
        var c = "150601" == queryConditions.transType ? "付" : "退";
        $("#grid").jqGrid({
            url: "../scm/invSd?action=list",
            ...
            gridview: !0,
            multiselect: !0,
            colNames: ["操作", "单据日期", "单据编号", "客户", "销售数量", "出库数量", "出库状态", /*"仓库",*/ "制单人", "审核人", "备注", "源单编号"],
            colModel: [{
                name: "operating",
                width: 60,
                fixed: !0,
    ...
    onShowSrc: function (srcId) {
        parent.tab.addTabItem({
            tabid: 'salesOrder',
            text: '销售订单',
            url: '../scm/invSo?action=editSo&flag=list&transType=150601&id=' + srcId
        });
    },
    ...
    addEvent: function () {
        ...
        if ($(".grid-wrap").on("click", ".ui-icon-pencil", function (a) {
                ...
                "150602" == queryConditions.transType ? (e = "销售退货单", f = "purchase-purchaseBack", parent.cacheList.purchaseBackId = $("#grid").jqGrid("getDataIDs")) : parent.cacheList.purchaseId = $("#grid").jqGrid("getDataIDs"),
                    parent.tab.addTabItem({
                        tabid: f,
                        text: e,
                        url: "../scm/invSd?action=editSd&id=" + b + "&flag=list" + d + "&transType=" + queryConditions.transType
                    })
            }), $(".grid-wrap").on("click", ".ui-icon-trash", function (a) {
                if (a.preventDefault(), Business.verifyRight("PUR_DELETE")) {
                    var b = $(this).parent().data("id");
                    $.dialog.confirm("您确定要删除该销售出库记录吗？", function () {
                        Public.ajaxGet("../scm/invSd/delete?action=delete", {
                            id: b
                        }, function (a) {
                ...
                var f = "../scm/invSd/exportInvSd?action=exportInvSd" + d;
                $(this).attr("href", f)
            }), billRequiredCheck) {
            {
            ...
            $(".wrapper").on("click", "#audit", function (a) {
                ...
                var b = $("#grid").jqGrid("getGridParam", "selarrrow"),
                    c = b.join();
                return c ? void Public.ajaxPost("../scm/invSd/batchCheckInvSd?action=batchCheckInvSd", {
                    id: c
                }, function (a) {
                    if (200 === a.status) {
                ...
                var b = $("#grid").jqGrid("getGridParam", "selarrrow"),
                    c = b.join();
                return c ? void Public.ajaxPost("../scm/invSd/rsBatchCheckInvSd?action=rsBatchCheckInvSd", {
                    id: c
                }, function (a) {
                    if (200 === a.status) {
        ...
        $("#search").click(function () {
            queryConditions.matchCon = "请输入单据号或供应商或备注" === a.$_matchCon.val() ? "" : a.$_matchCon.val(), queryConditions.beginDate = a.$_beginDate.val(), queryConditions.endDate = a.$_endDate.val(),
            ...
            if (a.preventDefault(), Business.verifyRight("PUR_ADD")) {
                ...
                parent.tab.addTabItem({
                    tabid: c,
                    text: b,
                    url: "../scm/invSd?action=initSd&transType=" + queryConditions.transType
                })
            }
