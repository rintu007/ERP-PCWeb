define(["jquery", "print"], function(a) {
	function b() {
		...
		j("#filter-submit").on("click", function(a) {
			...
			return c && d && new Date(c).getTime() > new Date(d).getTime() ? void parent.Public.tips({
				...
				profit: 1//b.chkVal().length > 0 ? "1" : "0"
	...
	function d() {
		j("#refresh").on("click", function(a) {
			...
			a.preventDefault(), Business.verifyRight("SAREPORTDETAIL_EXPORT") && Business.getFile("../report/salesProfit_Exporter?action=detailExporter", l)
		})
	}
	function e() {
		var a = !1,
		...
		var c = [{
			...
			d = "local",
			e = "#";
		l.autoSearch && (d = "json", e = "../report/salesProfit_detail?action=detail"), j("#grid").jqGrid({
			url: e,
			postData: l,
			datatype: d,
			autowidth: !0,
	...
	function i(a) {
		j(".no-query").remove(), j(".ui-print").show(), "number" == typeof a && (j("#grid").jqGrid(a ? "showCol" : "hideCol", ["unitCost", "cost", "saleProfit", "salepPofitRate"]), f(), j("#grid").clearGridData(!0)), j("#grid").jqGrid("setGridParam", {
			datatype: "json",
			postData: l,
			url: "../report/salesProfit_detail?action=detail"
		}).trigger("reloadGrid")
	}
	var j = a("jquery"),
		k = parent.SYSTEM,
