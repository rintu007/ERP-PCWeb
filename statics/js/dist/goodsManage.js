...
function initField() {
	$("#note").placeholder(), "edit" == oper ?  
			...
			void 0 != rowData.purPrice && $("#purchasePrice").val(Public.numToCurrency(rowData.purPrice, pricePlaces)),
			void 0 != rowData.salePrice && ($("#salePrice").val(Public.numToCurrency(rowData.salePrice, pricePlaces)), 
			$("#currency").val(rowData.currency),
			//$("#currency").data("defItem", ["id", rowData.currency]), 
			$("#wholesalePrice").val(Public.numToCurrency(rowData.wholesalePrice, pricePlaces)), 
			$("#vipPrice").val(Public.numToCurrency(rowData.vipPrice, pricePlaces)), 
			$("#discountRate1").val(rowData.discountRate1), $("#discountRate2").val(rowData.discountRate2)),
			$("#minInventory").val(rowData.lowQty), 
			$("#maxInventory").val(rowData.highQty), 
...
function initEvent() {
	...
	currencyCombo = $("#currency").combo({
		...
		text: "name",
		width: comboWidth,
		defaultSelected: ("add" == oper ? 0 : Public.ToCurrencyIndex($("#currency").val())),
	}).getCombo(),
...
function resetForm(a) {
	...
	$("#name").val(""), $("#specs").val(""), $("#purchasePrice").val(""), $("#salePrice").val(""), $("#wholesalePrice").val(""), $("#vipPrice").val(""), $("#discountRate1").val(""), $("#discountRate2").val(""), $("#lowQty").val(""), $("#highQty").val(""), $("#note").val(""), $grid.jqGrid("clearGridData", !0).jqGrid("setGridParam", {
		data: b
	}).trigger("reloadGrid"), gridStoCombo.collapse(), $("#barCode").val(""), jianxingCombo && jianxingCombo.selectByIndex(0), $("#length").val(""), $("#width").val(""), $("#height").val(""), $("#weight").val("")
}
...
var curRow, curCol, curArrears, api = frameElement.api,
	...
	categoryTree, storageCombo, unitCombo, currencyCombo,gridStoCombo, jianxingCombo, comboWidth = 147,
	gridWidth = 970,
	$grid = $("#grid"),
	$itemList = $("#itemList"),
	$form = $("#manage-form"),
