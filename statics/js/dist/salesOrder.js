var curRow, curCol, loading, SYSTEM = system = parent.SYSTEM,
    ...
    THISPAGE = {
        init: function (a) {
            "150602" == urlParam.transType ?
                this.mod_PageConfig = Public.mod_PageConfig.init("salesBack")
                : this.mod_PageConfig = Public.mod_PageConfig.init("salesOrder"),
            SYSTEM.isAdmin !== !1 || SYSTEM.rights.AMOUNT_OUTAMOUNT
            || (hiddenAmount = !0,
                $("#amountArea").hide()), this.initDom(a), this.loadGrid(a),
                this.initCombo(), a.id > 0 && a.checked ? this.disableEdit() :
        ...
        initDom: function (a) {
            ...
            this.salesCombo = Business.billSalesCombo($("#sales"), {
                ...
                this.customerCombo = Business.billCustomerCombo($("#customer"), {
                    ...
                    callback: {
                        onChange: function (a) {
                            a ? ($("#customer").data("contactInfo", a),
                                b.setSaleByContact(a)) : $("#customer").removeData("contactInfo")
                        }
                    }
                }),
                this.currencyCombo = Business.currencyCombo($("#currency"), {
                    defaultSelected: c
                }),
                this.currencyCombo.selectByValue(a.currency, !1);
            this.paymentCombo = Business.paymentCombo($("#payment"), {
                defaultSelected: 0
            });
            this.paymentCombo.selectByValue(a.paymentMethod, !1);
            this.shippingCombo = Business.shippingCombo($("#shipping"), {
                defaultSelected: 0
            });
            this.shippingCombo.selectByValue(a.shippingMethod, !1);

            "add" !== a.status || a.buId ? b.customerCombo.selectByValue(a.buId)
                ...
                onSelect: function (a) {
                    if (!(originalData.id > 0)) {
                        ...
                        b.$_number.text(""),
                            Public.ajaxPost("../basedata/systemProfile/generateDocNo?action=generateDocNo", {
                                billType: "SO",
                                billDate: c
                            }, function (a) {
                ...
                this.$_deliveryDate.val(a.deliveryDate),
                        ...
                        onChange: function () {
                            if (-1 === this.getValue()) b.chooseAccount();
                                ...
                                a.push({
                                    ...
                                    way: "",
                                    settlement: ""
                                }), b.$_accountInfo.data("accountInfo", a).hide()
            ...
            var d = '<a id="savaAndAdd" class="ui-btn ui-btn-sp">保存并新增</a><a id="save" class="ui-btn">保存</a>',
                e = '<a id="add" class="ui-btn ui-btn-sp">新增</a><a href="../scm/invSo/toPdf?action=toPdf&id=' + a.id + '&transType='+a.transType+'" target="_blank" id="print" class="ui-btn">打印</a><a id="edit" class="ui-btn">保存</a>',
                f = '<a id="add" class="ui-btn ui-btn-sp">新增</a><a href="../scm/invSo/toPdf?action=toPdf&id=' + a.id + '&transType='+a.transType+'" target="_blank" id="print" class="ui-btn">打印</a>',
                g = "",
                h = "";
                var i;
                if("150602" == originalData.transType)
                ...
                var j = '<a id="turn" class="ui-btn ' + (2 == a.billStatus ? "ui-btn-dis" : "ui-btn-sc") + '">' + i + "</a>";
            ...
            this.btn_edit = e,
                this.btn_audit = g, this.btn_view = f,
                this.btn_reaudit = h, this.btn_turn = j,
                a.id > 0 ? (this.$_number.text(a.billNo), this.$_date.val(a.date),
                    this.$_deliveryDate.val(a.deliveryDate),
                    //this.$_totalArrears.val(a.totalArrears),
                    this.$_accountInfo.data("accountInfo", a.accounts), -1 === a.accId &&
                (this.$_accountInfo.show()
                    ...
                    }), "list" !== urlParam.flag && (k = ""), "edit" === a.status ? this.$_toolBottom.html("<span id=groupBtn>" + e + g + "</span>" + k) : a.checked ? ($("#mark").addClass("has-audit"), this.$_toolBottom.html('<span id="groupBtn">' + j + f + h + "</span>" + k)) : this.$_toolBottom.html('<span id="groupBtn">' + f + "</span>" + k),
                    this.idList = parent.cacheList.salesOrderId || [],
                    this.idPostion = $.inArray(String(a.id),
                        this.idList),
                    this.idLength = this.idList.length, 0 === this.idPostion && $("#prev").addClass("ui-btn-prev-dis"), this.idPostion === this.idLength - 1 && $("#next").addClass("ui-btn-next-dis"), this.$_userName.html(a.userName), this.$_modifyTime.html(a.modifyTime), this.$_checkName.html(a.checkName)) : (this.$_toolBottom.html(billRequiredCheck ? "<span id=groupBtn>" + d + g + "</span>" : '<span id="groupBtn">' + d + "</span>"), this.$_userName.html(system.realName || ""), this.$_modifyTime.parent().hide(), this.$_checkName.parent().hide()), disEditable && (THISPAGE.disableEdit(), this.$_toolBottom.hide())
        },
        loadGrid: function (a) {
            function b(a) {
                if (taxRequiredCheck) {
                    ...
                    if (THISPAGE.$_barCodeInsert.hasClass("active")) {
                        var a = function (a) {
                            var b = $("#" + a),
                                c = b.next(),
                                d = b.index() + 1;
                            return 0 == c.length ? ($("#grid").jqGrid("addRowData", THISPAGE.newId, {}, "last"),
                                THISPAGE.newId++,
                                $("#" + (THISPAGE.newId - 1)).index()) : c.data("goodsInfo") ? arguments.callee(d) : d
                        }(THISPAGE.curID);
                    ...
                    if (THISPAGE.$_barCodeInsert = $("#barCodeInsert"), urlParam.id > 0) {
                        ...
                        for (var e = {}, f = 0; c > f; f++) {
                            ...
                            Business.cacheManage.getGoodsInfoByNumber(i.number, function (a) {
                                i.isSerNum = a.isSerNum,
                                    i.isWarranty = h.isWarranty = a.isWarranty,
                                    i.safeDays = h.safeDays = a.safeDays,
                                    i.invSkus = a.invSkus,
                                    i.id = h.invId, $("#" + g).data("goodsInfo", i).data("storageInfo", {
                                    id: h.locationId,
                                    name: h.locationName
                                }).data("unitInfo", {
                                    unitId: h.unitId,
                ...
                afterEditCell: function (a, b, c, d, e) {
                    if (THISPAGE.curID = a, "goods" === b) {
                        ...
                        if (f) {
                            ...
                            f = $.extend(!0, {}, f), f.skuName =
                                g.skuName,
                                f.mainUnit = g.mainUnit,
                                f.unitId = g.unitId,
                                f.qty = g.qty,
                                f.price = g.price,
                                f.discountRate = g.discountRate,
                                f.deduction = g.deduction,
                                f.amount = g.amount,
                                f.taxRate = g.taxRate,
                                f.tax = g.tax,
                                f.taxAmount = g.taxAmount,
                                f.locationName = g.locationName,
                                f.stockQty = g.stockQty,
                                $("#" + a).data("goodsInfo", f)
                        }
                    ...
                    switch (b) {
                        case "goods":
                            ...
                            if (!d) {
                                ...
                                return THISPAGE.$_barCodeInsert.hasClass("active") ?
                                    Business.cacheManage.getGoodsInfoByBarCode(c, f, !0)
                                    : Business.cacheManage.getGoodsInfoByNumber(c, f, !0),
                                    e ? e : ($.dialog({
                                        width: 775,
                                        height: 510,
                                        title: "选择商品",
        ...
        reloadData: function (a) {
            function b() {
                c.$_customer.data("contactInfo", {
                    ...
                    c.salesCombo.selectByValue(a.salesId, !1), c.$_date.val(a.date),
                    c.$_deliveryDate.val(a.deliveryDate), c.$_number.text(a.billNo),
                    c.$_note.val(a.description),
                    c.$_discountRate.val(a.disRate), c.$_deduction.val(a.disAmount), c.$_discount.val(a.amount),
                    c.$_payment.val(a.rpAmount),
                    c.accountCombo.selectByValue(a.accId, !1),
                    c.$_accountInfo.data("accountInfo", a.accounts), -1 === a.accId ? c.$_accountInfo.show() : c.$_accountInfo.hide(),
                    c.$_arrears.val(a.arrears), c.$_totalArrears.val(a.totalArrears),
                    c.$_userName.html(a.userName), c.$_modifyTime.html(a.modifyTime), c.$_checkName.html(a.checkName)
            }
        ...
        initCombo: function () {
            ...
            this.goodsCombo = Business.billGoodsCombo($(".goodsAuto"), {
                ...
                data: function () {
                    ...
                    // 	var f = "";
                    // 	a.cLevel < 3 ? f = ["零售", "批发", "VIP"][a.cLevel] + "价：" + d.prices.levelPrice :
                    // 		d.prices.discountRate && (f = ["折扣一", "折扣二"][a.cLevel - 3] + "价：" + d.prices.levelPrice * d.prices.discountRate / 100), f && e.push({
                    // 		name: f,
                    // 		id: 1
                    // 	})
                    // }
                                    ...
                                    f[d.id] = a, Public.ajaxPost("../basedata/inventory/listBySelected?action=listBySelected", {
                                        ...
                                        if (200 === c.status && c.data && c.data.result) {
                                            for (var d = c.data.result, e = 0, f = d.length; f > e; e++) {
                                                var g = d[e];
                                                g.nearPrice && (a.prices = {}, a.prices.nearPrice = g.nearPrice), g.salePrice && (a.prices = a.prices || {}, a.prices.levelPrice = g.salePrice
                                                    //, a.prices.discountRate = g.discountRate
                                                )
                                            }
                                            a.prices && b.show()
                                        }
        ...
        disableEdit: function () {
            this.customerCombo.disable(), this.salesCombo.disable(),
                this.$_date.attr("disabled", "disabled").addClass("ui-input-dis"),
                this.$_deliveryDate.attr("disabled", "disabled").addClass("ui-input-dis"), this.$_note.attr("disabled", "disabled").addClass("ui-input-dis"),
        ...
        enableEdit: function () {
            disEditable || (this.salesCombo.enable(), this.customerCombo.enable(), this.$_date.removeAttr("disabled").removeClass("ui-input-dis"), this.$_deliveryDate.removeAttr("disabled").removeClass("ui-input-dis"), this.$_note.removeAttr("disabled").removeClass("ui-input-dis"),
                ...
                this.accountCombo.enable(), $("#grid").jqGrid("setGridParam", {
                cellEdit: !0
            }), this.editable = !0)
        },
            ...
            b.$_accountInfo.show(),
                ...
                $.dialog({
                    ...
                    ok: function () {
                        ...
                        return a ? (
                            //b.$_payment.val(a.payment).trigger("keyup"),
                            b.$_accountInfo.data("accountInfo", a.accounts),
                                b.accountCombo.blur(), void 0) : !1
                    },
                    cancel: !0
                })
        },
        addEvent: function () {
            ...
            this.customerCombo.input.enterKey(), this.$_date.bind("keydown", function (b) {
                ...
                this.$_discountRate.enterKey(), $(".grid-wrap").on("click", ".ui-icon-triangle-1-s", function () {
                var a = $(this).siblings(),
                    b = a.getCombo();
                setTimeout(function () {
                ...
                d && c.ajaxPost("../scm/invSo/addNew?action=addNew", {
                    ...
                    if (200 === b.status) {
                        ...
                        a.newId = 9,
                            ...
                            a.accountCombo.selectByValue(0, !0), parent.Public.tips({
                            content: "保存成功！"
                        })
                ...
                if (b.preventDefault(), Business.verifyRight("SO_CHECK")) {
                    ...
                    d && c.ajaxPost("../scm/invSo/checkInvSo?action=checkInvSo", {
                        ...
                        200 === b.status ? (originalData.id = b.data.id,
                            $("#mark").addClass("has-audit"), $("#edit").hide(),
                            a.$_checkName.html(SYSTEM.realName).parent().show(),
                            a.disableEdit(), $("#groupBtn").html(a.btn_turn + a.btn_view + a.btn_reaudit),
                            $("#turn").html("150602" == originalData.transType ? "生成退货单" : "生成销售单"),
                            parent.Public.tips({
                            content: "审核成功！"
                        })) : parent.Public.tips({
                            type: 1,
                ...
                if (b.preventDefault(), $(this).hasClass("ui-btn-dis")) return void parent.Public.tips({
                    type: 1,
                    content: "该订单已全部入库，不能生成销售单！"
                });
                var c = "销售单",
                    d = "sales-sales",
                    e = originalData.transType;
                if ("150602" == e) var c = "销售退货单",
                    d = "sales-salesBack";
                parent.tab.addTabItem({
                    tabid: d,
                    text: c,
                    url: "../scm/invSa?action=initSale&id=" + originalData.id + "&flag=list&turn&transType=" + originalData.transType
                }), parent.tab.reload(d)
            }), $("#prev").click(function (b) {
                return b.preventDefault(), $(this).hasClass("ui-btn-prev-dis") ? (parent.Public.tips({
                    type: 2,
                    content: "已经没有上一张了！"
                }), !1) : (a.idPostion = a.idPostion - 1, 0 === a.idPostion && $(this).addClass("ui-btn-prev-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0),
                ...
                return b.preventDefault(), $(this).hasClass("ui-btn-next-dis") ? (parent.Public.tips({
                    type: 2,
                    content: "已经没有下一张了！"
                }), !1) : (a.idPostion = a.idPostion + 1, a.idLength === a.idPostion + 1 && $(this).addClass("ui-btn-next-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0),
                    Public.ajaxGet("../scm/invSo/update?action=update", {
                    id: a.idList[a.idPostion]
                }, function (a) {
        ...
        calTotal: function () {
            ...
            if ($("#grid").jqGrid("footerData", "set", {
                ...
                var k = (f - Number(this.$_deduction.val())).toFixed(2);
        ...
        getPostData: function () {
            ...
            if (isNaN(parseInt(a.$_deliveryDate.val()))) {
                parent.Public.tips({
                    type: 2,
                    content: "请输入交货周期"
                });
            ...
            if (this.currencyCombo.getValue() <= 0) {
                parent.Public.tips({
                    type: 2,
                    content: "请选择交易币种"
                });
            ...
            if (this.paymentCombo.getValue() <= 0) {
                parent.Public.tips({
                    type: 2,
                    content: "请选择支付方式"
                });
            ...
            if (this.shippingCombo.getValue() <= 0) {
                parent.Public.tips({
                    type: 2,
                    content: "请选择交货方式"
                });
            ...
            if (e.length > 0) {
                var f = $.trim(a.$_note.val()),
                    g = {
                        ...
                        staffName: a.salesCombo.getText(),
                        ...
                        currency: this.currencyCombo.getValue(),
                        accId: a.accountCombo.getValue(),
                        paymentMethod: this.paymentCombo.getValue(),
                        shippingMethod: this.shippingCombo.getValue()
                    };
...
$(function () {
    if (urlParam.id) {
        if (!hasLoaded) {
            ...
            Public.ajaxGet("../scm/invSo/update?action=update", {
                id: urlParam.id,
                transType: urlParam.transType?urlParam.transType:'150601'
            }, function (b) {
