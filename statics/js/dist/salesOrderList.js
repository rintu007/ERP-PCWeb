var queryConditions = {
	...
	THISPAGE = {
		...
		loadGrid: function() {
			...
			function b(a, b, c) {
				if(150601 === a)return "销售";
                else if(150603 === a)return "报价";
                else if(150602 === a)return "退货";
                return "";
			}
			...
			queryConditions.beginDate = this.$_beginDate.val(),
                ...
                queryConditions.transType = urlParam.transType,
				d.markRow = [];
			var e = [{
				name: "operating",
				label: "操作",
				...
				align: "center",
                width: 60,
                hidden: (urlParam.transType==="150602"||urlParam.transType==="150603") ? true : false
			}, {
				name: "deliveryDate",
				label: "交货周期",
				index: "deliveryDate",
		...
		addEvent: function() {
			...
			if ($(".grid-wrap").on("click", ".ui-icon-pencil", function(a) {
				...
				e="&transType="+urlParam.transType;
				parent.tab.addTabItem({
					tabid: "sales-salesOrder",
					text: "销售订单",
					url: "../scm/invSo?action=editSo&id=" + b + "&flag=list" + d + e
				});
			...
			$("#search").click(function() {
				queryConditions.salesId = null,
					queryConditions.matchCon = "请输入单据号或客户名或备注" === a.$_matchCon.val() ? "" : $.trim(a.$_matchCon.val()),
