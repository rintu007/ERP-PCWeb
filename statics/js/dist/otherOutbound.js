	...
	THISPAGE = {
		...
		initDom: function(a) {
			...
			var c = "",
				d = "",
				e = "";
			if (billRequiredCheck && (c = '<a class="ui-btn" id="audit">审核</a>', d = '<a class="ui-btn" id="reAudit">反审核</a>'), this.btn_audit = c, this.btn_reaudit = d, this.btn_tempSave = e, this.btn_add = '<a id="savaAndAdd" class="ui-btn ui-btn-sp mrb">保存并新增</a><a id="save" class="ui-btn">保存</a>' + e, this.btn_edit = '<a id="add" class="ui-btn ui-btn-sp mrb">新增</a><a href="../scm/invOi/toOoPdf?action=toOoPdf" target="_blank" id="print" class="ui-btn mrb">打印</a><a id="edit" class="ui-btn">保存</a>', this.btn_view = '<a id="add" class="ui-btn ui-btn-sp mrb">新增</a><a href="../scm/invOi/toOoPdf?action=toOoPdf" target="_blank" id="print" class="ui-btn mrb">打印</a>', this.btn_p_n = '<a class="ui-btn-prev mrb" id="prev" title="上一张"><b></b></a><a class="ui-btn-next" id="next" title="下一张"><b></b></a>', a.id > 0) {
				...
				this.salesListIds = parent.salesListIds || [], this.idPostion = $.inArray(String(a.id), this.salesListIds), this.idLength = this.salesListIds.length, 0 === this.idPostion && $("#prev").addClass("ui-btn-prev-dis"), this.idPostion === this.idLength - 1 && $("#next").addClass("ui-btn-next-dis"), this.$_userName.html(a.userName)
			} else this.$_toolBottom.html(billRequiredCheck ? this.btn_add + this.btn_audit : this.btn_add), this.$_userName.html(SYSTEM.realName || "")
		},
		...
		loadGrid: function(a) {
			function b(a, b, c) {
				return r(b.rowId), a ? a : c.invNumber ? c.invSpec ? c.invNumber + " " + c.invName + "_" + c.invSpec : c.invNumber + " " + c.invName : "&#160;"
			}
			function c() {
				var a = $(".goodsAuto")[0];
				return a
			...
			s.mod_PageConfig.gridReg(w, x), x = s.mod_PageConfig.conf.grids[w].colModel, $("#grid").jqGrid({
				...
				afterEditCell: function(a, b, c, d, e) {
					function f() {
						...
						if (b) {
							...
							b = $.extend(!0, {}, b),
								...
								b.unitId = c.unitId, b.qty = c.qty,
								b.price = c.price, b.discountRate = c.discountRate,
								b.deduction = c.deduction,
							b.amount = c.amount, b.taxRate = c.taxRate, b.tax = c.tax,
								b.taxAmount = c.taxAmount, b.locationName = c.locationName,
								$("#" + a).data("goodsInfo", b)
						}
					}
					...
					if ("locationName" === b) {
						$("#" + d + "_locationName", "#grid").val(c), THISPAGE.storageCombo.selectByText(c), f();
						var g = $("#" + a).data("goodsInfo"),
				...
				afterSaveCell: function(a, b, c) {
					...
					case "batch":
						...
						if (e.safeDays) {
							var f = {};
							if ($.trim(d.prodDate) || (f.prodDate = t), $.trim(d.safeDays) || (f.safeDays = e.safeDays), !$.trim(d.validDate)) {
					...
					case "prodDate":
						var d = $("#grid").jqGrid("getRowData", a),
							e = $("#" + a).data("goodsInfo") || {},
							f = {};
						$.trim(d.safeDays) || (f.safeDays = e.safeDays || 30), $.trim(c) || (f.prodDate = t);
						var g = c || f.prodDate,
							h = g.split("-");
						if (g = new Date(h[0], h[1] - 1, h[2]), "Invalid Date" === g.toString()) return defaultPage.Public.tips({
							type: 2,
				...
				resizeStop: function(a, b) {
					s.mod_PageConfig.updatePageConfig("grid", ["width", s.mod_PageConfig.conf.grids.grid.defColModel[b - 1].name, a])
				},
				footerrow: !0,
				userData: {
					goods: "合计：",
		...
		initCombo: function() {
			this.goodsCombo = Business.billGoodsCombo($(".goodsAuto")), this.storageCombo = Business.billStorageCombo($(".storageAuto")), this.unitCombo = Business.unitCombo($(".unitAuto"), {
				defaultSelected: -1,
				forceSelection: !1
		...
		addEvent: function() {
			...
			this.customerCombo.input.enterKey(), this.$_date.bind("keydown", function(a) {
				...
				if (a.preventDefault(), Business.verifyRight("OO_UPDATE")) {
					var b = THISPAGE.getPostData();
					b && Public.ajaxPost("../scm/invOi/updateOo?action=updateOo&type=out", {
				...
				if (b.preventDefault(), Business.verifyRight("OO_UNCHECK")) {
					var c = THISPAGE.getPostData();
					c && Public.ajaxPost("../scm/invOi/revsCheckInvOo?action=revsCheckInvOo", {
						postData: JSON.stringify(c)
					}, function(b) {
