define(["jquery", "print"], function(a) {
	function b() {
		Business.filterSettlementAccount(), i("#filter-fromDate").val(k.beginDate || ""), i("#filter-toDate").val(k.endDate || ""), k.beginDate && k.endDate && i("div.grid-subtitle").text("日期: " + k.beginDate + " 至 " + k.endDate), i("#filter-fromDate, #filter-toDate").datepicker(), Public.dateCheck(), i("#filter-submit").on("click", function(a) {
			a.preventDefault();
			...
			return b && c && new Date(b).getTime() > new Date(c).getTime() ? void parent.Public.tips({
				...
				beginDate: b,
				endDate: c,
				accountNo: i("#settlementAccountAuto").val() || ""
			}, i("div.grid-subtitle").text("日期: " + b + " 至 " + c), void h(1))
		})
	}
	function c() {
		i("#btn-print").click(function(a) {
			a.preventDefault(), Business.verifyRight("SettAcctReport_PRINT") && i("div.ui-print").printTable()
	...
	var i = a("jquery"),
		...
		l = "../report/bankBalance_exporter?action=exporter",
		m = "../report/bankBalance_detail?action=detail";
	a("print"), b(), c(), d();
	var n;
