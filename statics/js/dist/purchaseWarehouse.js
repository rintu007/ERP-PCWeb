var curRow, curCol, loading, SYSTEM = system = parent.SYSTEM, billRequiredCheck = system.billRequiredCheck, hideCustomerCombo = !1, urlParam = Public
	...
	init : function(a) {
		this.mod_PageConfig = Public.mod_PageConfig.init("purchaseWarehouse"), SYSTEM.isAdmin !== !1, this
	...
	initDom : function(a) {
		...
		$("#orderid").autocomplete({
			...
			source : function(request, response) {
				Public.ajaxPost("../scm/invPw?action=unStorageList", {},
						function(a) {
							if (null !== a) {
								...
								response($.grep(a, function(item) {
											return matcher.test(item.billNo);
										}));
							}
						});
			},
		...
		var c = '<a id="save" class="ui-btn">保存</a>', d = '<a href="../scm/invPw/toPdf?action=toPdf&id='
				+ a.id
				+ '" target="_blank" id="print" class="ui-btn">打印</a><a id="edit" class="ui-btn">保存</a>', e = '<a href="../scm/invPw/toPdf?action=toPdf&id='
	...
	loadOrder : function(billNo) {
		...
		Public.ajaxGet("../scm/invPw/order?action=order", {
					billNo : billNo
				}, function(b) {
					...
					afterEditCell : function(a, b, c, d, e) {
						function f() {
							...
							if (b) {
								...
								b = $.extend(!0, {}, b), b.mainUnit = c.mainUnit, b.qty = c.qty, b.locationName = c.locationName, b.stockinQty = c.stockinQty, $("#"
										+ a).data("goodsInfo", b)
							}
						}
						if ("goods" === b
						...
						if ("locationName" === b
								&& ($("#" + d + "_locationName", "#grid")
										...
										.selectByText(c), "150502" == originalData.transType)) {
							f();
	...
	disableEdit : function() {
		this.$_date.attr("disabled", "disabled").addClass("ui-input-dis"), this.$_note
				.attr("disabled", "disabled").addClass("ui-input-dis"),
	...
	enableEdit : function() {
		disEditable
				|| (this.$_date.removeAttr("disabled")
						...
						.removeAttr("disabled").removeClass("ui-input-dis"))
	},
	addEvent : function() {
		var a = this;
		this.$_date.bind("keydown", function(a) {
			...
			b.test($(this).val()) || (parent.Public.tips({
				...
				.on("click", "#save", function(b) {
					...
					d
							&& ("edit" === originalData.stata
									&& (d.id = originalData.id, d.stata = "edit"), c
									.ajaxPost("../scm/invPw/add?action=add", {
			...
			if (b.preventDefault(), Business.verifyRight("PUR_UPDATE")) {
				var c = $(this), d = THISPAGE.getPostData();
				d
						&& c.ajaxPost(
								"../scm/invPw/updateInvPw?action=updateInvPw",
								{
									postData : JSON.stringify(d)
								}, function(b) {
									200 === b.status
			...
			if (b.preventDefault(), Business.verifyRight("PUR_CHECK")) {
				var c = $(this), d = THISPAGE.getPostData({
							checkSerNum : !0
						});
				d && c.ajaxPost("../scm/invPw/checkInvPw?action=checkInvPw", {
							postData : JSON.stringify(d)
						}, function(b) {
							200 === b.status
			...
			if (b.preventDefault(), Business.verifyRight("PUR_UNCHECK")) {
				var c = $(this), d = THISPAGE.getPostData();
				d
						&& c
								.ajaxPost(
										"../scm/invPw/revsCheckInvPw?action=revsCheckInvPw",
										{
											postData : JSON.stringify(d)
										}, function(b) {
											200 === b.status
			...
			a.preventDefault(), Business.verifyRight("PUR_PRINT")
					&& Public.print({
								title : "采购入库单列表",
								$grid : $("#grid"),
			...
			return b.preventDefault(), $(this).hasClass("ui-btn-prev-dis")
					...
					: (a.idPostion = a.idPostion - 1, 0 === a.idPostion
							...
							.ajaxGet("../scm/invPw/order?action=order", {
	...
	if (urlParam.id) {
		if (!hasLoaded) {
			...
			urlParam.turn ? Public.ajaxGet(
					"../scm/invPw/queryDetails?action=queryDetails", {
						id : urlParam.id
					}, function(b) {
						200 === b.status
								...
								: (parent.Public.tips({
									...
									arrears : "0.00",
									accId : 0
								}, originalData.transType = "150502" === urlParam.transType
										? "150502"
										: "150501", THISPAGE.init(originalData))
					})
					: Public.ajaxGet("../scm/invPw/getPurchaseWarehouse?action=getPurchaseWarehouse", {
								id : urlParam.id
							}, function(b) {
								if(200 === b.status)
										{
