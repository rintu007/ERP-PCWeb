...
//将币种数值转换为文字
Public.ToCurrencyText = function(a){
	if(a==undefined)return "";
	var b = a;
	$.each(parent.parent.SYSTEM.currencyInfo,function(key,value){  
		if(value.id==a){					    	
		   	b = value.name;
		   	return true;
	    }					    
...
//根据币种id获取顺序
Public.ToCurrencyIndex = function(a){
	if(a==undefined)return "";
	var ca = parent.parent.SYSTEM.currencyInfo;
	if(ca==null)return -1;
...
//根据币种id获取币种Code
Public.ToCurrencyCode = function(a){
	if(a==undefined)return "";
	var ca = parent.parent.SYSTEM.currencyInfo;
	if(ca==null)return -1;
	for (var i=0;i<ca.length;i++)
...
//jason.xie add 2017/10/21
Business.printTemplateCombo = function($_obj, opts){
	var opts = $.extend(true, {
		data: [{
		    "id":1,
		    "name":"成品模板"
...
Business.storageCombo = function($_obj, opts){
    ...
    var opts = $.extend(true, {
        data: function(){
            if(SYSTEM.storageInfo) {
                return SYSTEM.storageInfo;
            } else {
                return '../basedata/invlocation?action=list&isDelete=0';
            }
        },
        ajaxOptions: {
            formatData: function(data){
                SYSTEM.storageInfo = data.data.items;	//更新缓存
                return data.data.items;
            }
        },
        width: 150,
...
Business.currencyCombo = function($_obj, opts){
	if ($_obj.length == 0) { return };
	var opts = $.extend(true, {
		data: function(){
			if(SYSTEM.currencyInfo) {
				return SYSTEM.currencyInfo;
			} else {
				return '../basedata/currency?action=list&isDelete=0';
			}
		},
		ajaxOptions: {
			formatData: function(data){
				SYSTEM.currencyInfo = data.data.items;	//更新缓存
				return data.data.items;
			}	
		},
		width: 80,
	...
	var currencyCombo = $_obj.combo(opts).getCombo();	
...
Business.paymentCombo = function($_obj, opts){
	...
	var opts = $.extend(true, {
		data: function(){
			if(SYSTEM.paymentMethod) {
				return SYSTEM.paymentMethod;
			} else {
		...
		ajaxOptions: {
			formatData: function(data){
				SYSTEM.paymentMethod = data.data.items;	//更新缓存
				return data.data.items;
			}	
		},
		width: 150,
...
Business.shippingCombo = function($_obj, opts){
	if ($_obj.length == 0) { return };
	var opts = $.extend(true, {
		data: function(){
			if(SYSTEM.shippingMethod) {
				return SYSTEM.shippingMethod;
			} else {
				return '../basedata/assist?action=list&typeNumber=ShippingMethod&isDelete=2';
			}
		},
		ajaxOptions: {
			formatData: function(data){
				SYSTEM.shippingMethod = data.data.items;	//更新缓存
				return data.data.items;
			}	
		},
		width: 80,
	...
	var shippingCombo = $_obj.combo(opts).getCombo();	
	return shippingCombo;
};
		...
		$('.grid-wrap').on('click', '.goods .ui-icon-ellipsis', function(e){
			...
			$.dialog({
					...
					data: {
						skuMult: skuMult,
						skey:_self.skey,
						//matchCon:$("#matchCon")?$("#matchCon").val():"",
...
//		cancel: true
//	});
	Business.getFile(pdfUrl, data, true, false);
};
//生成树
Public.zTree = {
    zTree: {},
...
Public.gotoDetailItem = function(params){
	...
	var urls = {
		//SALE
		'150601' : {tabid : 'sales-sales',text : '销售出库单',right:'SA_QUERY', url : '/sales/sales.jsp?id='},
		//SALEBACK
		'150602' : {tabid : 'sales-salesBack',text : '销售退货单',right:'SA_QUERY', url : '/sales/sales.jsp?transType=150602&id='},
		//PUR
		'150501' : {tabid : 'purchase-purchase',text : '采购入库单',right:'PU_QUERY',url : '/purchase/purchase.jsp?id='},
		//PUR
