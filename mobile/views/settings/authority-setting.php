...
<script>
  ...
  var relation = {
			"采购单":[{name:'商品',rights:['查询']},{name:'仓库',rights:['查询']},{name:'供应商',rights:['查询']}],
			"销售单":[{name:'商品',rights:['查询']},{name:'仓库',rights:['查询']},{name:'客户',rights:['查询']}],
  ...
  $('.grid-wrap').on('click', '.ckbox', function(){
	 ...
	 var $_group = $('.ckbox[data-for=' + groupId + ']'), $_view = $_group.filter('[data-view=true]'),
         $_others = $_group.not('[data-view=true]');
	 if(!$(this).data('view')) {
 ...
 function setRelativeRight(view){
	 ...
	 if(relation[_modelName]){
		 view.on('checkChange',function(){
             var modelName = $(this).closest('.jqgrow').find('td:eq(1)').html();
             var _arr = relation[modelName];
			 ...
			 for(var i = 0,len = _arr.length; i < len; i++){
				 ...
				 for(var j=0,l = _rights.length; j<l; j++){
					 ...
					 if(_isChecked){
						 //如果主权限获得，则做以下处理
                         if(_arr[i].ckbox[_proName] != undefined) {
