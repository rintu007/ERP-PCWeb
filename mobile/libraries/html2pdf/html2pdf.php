...
'.$content.'
        ...
        protected function pdfStrCut($str='',$cellWidth=0) {
            if($cellWidth==0)return array($str);
            $add= array();
            $wordWidth=0;
            $i=0;
            preg_match_all("/./u",$str,$arrTem);
            $arr=$arrTem[0];
            $count=count($arr);
            for($j=0;$j<$count;$j++){
                $wordWidth+=$this->pdf->GetCharWidth($arr[$j]);
        ...
        protected function _tag_open_WRITE($param)
        {
            ...
            $txt = $param['txt'];
            if(strlen(trim($txt))==0)return true;
            ...
            $w = 0;
            //$words = explode(' ', $txt);
            $words=$this->pdfStrCut($txt,$this->parsingCss->value['width']);
