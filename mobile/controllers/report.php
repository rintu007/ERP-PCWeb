...
class Report extends CI_Controller {
	...
	public function sales_profit() {
		$this->common_model->checkpurview(10000);
		$this->load->view('report/sales-profit');
	}
	
	//销售利润表接口 jason.xie add
	public function salesProfit_detail() {
		$this->common_model->checkpurview(10000);
		$v = array();
		$sum1 = $sum2 = $sum3 = $sum4 = $sum5 = $sum6 = $sum7 = 0;
		$data['status'] = 200;
		...
		foreach ($list as $arr=>$row) {
			...
			//jason.xie amend
			if ($profit==1) {
				$v[$arr]['cost']          = str_money($cost,2);//销售成本
				$v[$arr]['unitCost']      = str_money($unitcost,2);//单位成本
				$v[$arr]['saleProfit']    = str_money($saleProfit,2);//销售毛利
				$v[$arr]['salepPofitRate']= round($salepPofitRate,2).'%';//销售毛利率
			}
		...
		//$data['data']['userdata']['unitCost']    = $sum4;
		$data['data']['userdata']['cost']        = str_money($sum5,2);
		$saleProfit=$sum3 - $sum5;
		$data['data']['userdata']['saleProfit']      = str_money($saleProfit,2);
		$salepPofitRate=$sum3>0 ? ($saleProfit/$sum3)*100 :0;
		$data['data']['userdata']['salepPofitRate']  = round($salepPofitRate,2).'%';
		die(json_encode($data));
	}
	
	//销售利润表（导出）jason.xie add
	public function salesProfit_Exporter() {
		$this->common_model->checkpurview(10000);
		$name = '销售利润表_'.date('YmdHis').'.xls';
		sys_csv($name);
		$this->common_model->logs('销售利润表导出:'.$name);
		$salesId    = str_enhtml($this->input->get_post('salesId',TRUE));
		$storageNo  = str_enhtml($this->input->get_post('storageNo',TRUE));
		$customerNo = str_enhtml($this->input->get_post('customerNo',TRUE));
		$goodsNo    = str_enhtml($this->input->get_post('goodsNo',TRUE));
		...
		$where .= $beginDate ? ' and a.billDate>="'.$beginDate.'"' : '';
		$where .= $endDate ? ' and a.billDate<="'.$endDate.'"' : '';
		$list = $this->data_model->get_invoice_info($where.' order by a.id');		
		//jason.xie add
		$info  = $this->data_model->get_invoice_info_ini('and billDate<="'.$endDate.'" group by invId');
		foreach ($list as $arr=>$row) {
			$sum1 += $qty = $row['qty']>0 ? -abs($row['qty']):abs($row['qty']);   //销售在数据库中是负数 在统计的时候应该是正数
			$list[$arr]['qty']=$qty;
			$sum3 += $amount = $row['amount'];                      //销售收入
			$list[$arr]['amount']=str_money($amount,2);
			$list[$arr]['price']=str_money($row['price'],2);               //单位成本
			...
			$list[$arr]['cost']          = str_money($cost,2);//销售成本
			$list[$arr]['unitCost']      = str_money($unitcost,2);//单位成本
			$list[$arr]['saleProfit']    = str_money($saleProfit,2);//销售毛利
			$list[$arr]['salepPofitRate']= round($salepPofitRate,2).'%';//销售毛利率
		}
		$data['list']=$list;		
		
		$data['sumqty']         = $sum1;		
		$data['amount']      = str_money($sum3,2);
		$data['cost']        = str_money($sum5,2);
		$saleProfit=$sum3 - $sum5;
		$data['saleProfit']      = str_money($saleProfit,2);
		$salepPofitRate=$sum3>0 ? ($saleProfit/$sum3)*100 :0;
		$data['salepPofitRate']  = round($salepPofitRate,2).'%';
		
		
		$this->load->view('report/salesProfit-exporter',$data);
	}
	...
	public function deliverDetail() {
		...
		$where2 .= $storageNo ? ' and d.number in('.str_quote($storageNo).')' : '';
