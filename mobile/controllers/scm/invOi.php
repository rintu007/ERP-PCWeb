...
class InvOi extends CI_Controller {
    ...
    public function toOiPdf() {
		...
		if (count($data)>0) { 
			...
			try {
				...
				$html2pdf->Output('toOiPdf_'.date('ymdHis').'.pdf','I');
	...
	//批量审核
	public function batchcheckinvOi() {
		$this->common_model->checkpurview(100);
		$id   = str_enhtml($this->input->post('id',TRUE));
		$data = $this->mysql_model->get_results(INVOICE,'(id in('.$id.')) and billType="OI" and checked=0');
		if (count($data)>0) {
			$info['checked']   = 1;
			$info['checkName'] = $this->jxcsys['name'];
			$sql = $this->mysql_model->update(INVOICE,$info,'(id in('.$id.'))');
			if ($sql) {
				$billno = array_column($data,'billNo');
				$billno = join(',',$billno);
				$this->common_model->logs('单据编号：'.$billno.'的单据已被审核！');
	...
	public function rsbatchcheckinvOi() {
		...
		$id   = str_enhtml($this->input->post('id',TRUE));
		$data = $this->mysql_model->get_results(INVOICE,'(id in('.$id.')) and billType="OI" and checked=1');
		if (count($data)>0) {
			$info['checked']   = 0;
			$info['checkName'] = '';
			$sql = $this->mysql_model->update(INVOICE,$info,'(id in('.$id.'))');
	...
	//单个反审核
	public function revscheckinvOi() {
		$this->common_model->checkpurview(101);
		$data = $this->input->post('postData',TRUE);
		if (strlen($data)>0) {
			$data = (array)json_decode($data, true);
    ...
    public function toOoPdf() {
		...
		if (count($data)>0) { 
			...
			try {
				...
				$html2pdf->pdf->SetDisplayMode('fullpage');
				$html2pdf->writeHTML($content, isset($_GET['vuehtml']));
				$html2pdf->Output('toOoPdf_'.date('ymdHis').'.pdf','I');
			}catch(HTML2PDF_exception $e) {
				echo $e;
				exit;
			}  
    ...
    public function toCBTZPdf() {
		...
		if (count($data)>0) { 
			...
			try {
				...
				$html2pdf->pdf->SetDisplayMode('fullpage');
				$html2pdf->writeHTML($content, isset($_GET['vuehtml']));
				$html2pdf->Output('exemple_cn.pdf','I');
			}catch(HTML2PDF_exception $e) {
				echo $e;
				exit;
			}  
