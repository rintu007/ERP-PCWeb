...
class Data_model extends CI_Model{
    ...
    public function get_order($where='',$type=2) {
        $sql = 'select
				...
				where
					(a.isDelete=0) '.$where;
        //exit($sql);
        return $this->mysql_model->query(ORDER,$sql,$type);
    }

    public function get_order_info($where='',$type=2) {
        $sql = 'select
		            a.*,
					b.name as invName, b.number as invNumber, b.spec as invSpec, b.unitName as mainUnit,b.remark as remark,
					b.currency as currency,f.currencyCode as currencyCode,f.currencyText as currencyText,
					c.number as contactNo, c.name as contactName,
					d.name as locationName ,d.number as locationNo,
					e.number as staffNo ,e.name as staffName
				from '.ORDER_INFO.' as a
					left join
						(select
							id,name,number,spec,unitName,currency,remark
						from '.GOODS.'
						where (isDelete=0)
						order by id desc) as b
					on a.invId=b.id
				...
				'.$where;
        //exit($sql);
        return $this->mysql_model->query(ORDER_INFO,$sql,$type);
    }

    public function get_sale($where='',$type=2) {
        $sql = 'select
		            a.*,
					b.name as contactName,
					b.number as contactNo,
	    			b.linkMans as linkMans,
					c.number as staffNo ,c.name as staffName,c.mobile as staffMobile,
				...
				where
					(a.isDelete=0) '.$where;
        //exit($sql);
        return $this->mysql_model->query(SALE,$sql,$type);
    }

    public function get_sale_info($where='',$type=2) {
        $sql = 'select
		            a.*,
					b.name as invName, b.number as invNumber, b.spec as invSpec, b.unitName as mainUnit,b.remark as remark,
					b.currency as currency,f.currencyCode as currencyCode,f.currencyText as currencyText,c.number as contactNo, c.name as contactName,
					d.name as locationName ,d.number as locationNo,
					e.number as staffNo ,e.name as staffName
				from '.SALE_INFO.' as a
					left join
				...
				'.$where;
        //exit($sql);
        return $this->mysql_model->query(SALE_INFO,$sql,$type);
    }
    ...
    public function get_invps($where='',$type=2) {
        $sql = 'select 
					...
					b.name as contactName,
					b.number as contactNo,   
					c.number as staffNo ,c.name as staffName
				from '.INVPS.' as a 
					left join 
						(select 
							id,number, name
    ...
    public function get_goods_ini($where='',$beginDate,$endDate,$type=2) {
        $sql = 'select 
					...
					c.name as locationName ,c.number as locationNo
				from '.INVOICE_INFO.' as a 
					left join 
						(select 
