...
class Invlocation extends CI_Controller {
	...
	public function add(){
		...
		if (count($data)>0) {
			...
			$count = $this->mysql_model->query(STORAGE,"SELECT MAX(id) as numrows FROM ".STORAGE,1);
			$data['number'] = '95ZYST'.str_pad($count[numrows]+1,4,"0",STR_PAD_LEFT);
			//end			
			$sql = $this->mysql_model->insert(STORAGE,
					elements(array('name','number','address','manager','email','phone'),$data));
			if ($sql) {
				$data['id'] = $sql;
	...
	public function update(){
		...
		if (count($data)>0) {
			...
			$data = $this->validform($data);			
			$sql = $this->mysql_model->update(STORAGE,elements(array('name','address','manager','email','phone'),$data),'(id='.$id.')');
			if ($sql) {
				$data['id'] = $id;
