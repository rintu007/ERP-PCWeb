...
class Inventory extends CI_Controller {
	...
	public function add(){
		...
		if ($data) {
			...
			$info = array(
				...
				'locationName','lowQty','name','number','purPrice','currency',
	...
	private function validform($data) {
        ...
        {
            //数据库已经存在相同的型号商品
            if(empty($data['id'])||$id!=$data['id']){str_alert(-1,'相同型号的商品已经存在');}
        }
//        $id=$this->mysql_model->get_row(GOODS,"(isDelete=0) and (name='$data[name]')",'id');
//        if(!empty($id))
//        {
//            exit($id.":".$data['id']);
//            //数据库已经存在相同的名称商品
//            if(empty($data['id'])||$id!=$data['id']){str_alert(-1,'相同名称的商品已经存在');}
//        }
