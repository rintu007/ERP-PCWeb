...
class Contact extends CI_Controller {
	...
	public function query() {    
		...
		if (count($data)>0) {
			...
			//jason.xie 2017/10/01
			$info['bank']           = isset($data['bank']) ? $data['bank'] : '';
			$info['account']           = isset($data['account']) ? $data['account'] : '';
			$info['taxnumber']           = isset($data['taxnumber']) ? $data['taxnumber'] : '';
			if (strlen($data['linkMans'])>0) {                            
				...
				foreach ($list as $arr=>$row) {
					...
					$v[$arr]['linkTitle']           = isset($row['linkTitle']) ? $row['linkTitle'] : '';
					...
					$v[$arr]['fax']        = isset($row['linkFax']) ? $row['linkFax'] : '';
					$v[$arr]['zipcode']        = isset($row['linkZipcode']) ? $row['linkZipcode'] : '';
					if ($row['linkFirst']==1) {
						//首要联系人的手机、座机、地址为该公司联系方式
						$info['contacter']        = isset($row['linkName']) ? $row['linkName'] : ''; 
						$info['linkTitle']           = isset($row['linkTitle']) ? $row['linkTitle'] : '';
						$info['mobile']           = isset($row['linkMobile']) ? $row['linkMobile'] : ''; 
						$info['telephone']        = isset($row['linkPhone']) ? $row['linkPhone'] : ''; 
						$info['linkIm']           = isset($row['linkIm']) ? $row['linkIm'] : '';
						$info['fax']        = isset($row['linkFax']) ? $row['linkFax'] : '';
						$info['zipcode']        = isset($row['linkZipcode']) ? $row['linkZipcode'] : '';
						
						$info['province']           = $row['province'];
						$info['city']           = $row['city'];
						$info['county']   = $row['county'];
						$info['deliveryAddress']           = $row['address'];
					}
				} 
		    }
			$info['links']  = $v;
	...
	public function add(){
		...
		$data = array_filter(elements(array(
					'name','amount','beginDate','cCategory',
					'cCategoryName','cLevel','cLevelName','linkMans'
					,'periodMoney','remark','type','difMoney','bank','account','taxnumber')
					,$data));
		//jason.xie 改为系统自建编号
		$count = $this->mysql_model->query(CONTACT,"SELECT MAX(id) as numrows FROM ".CONTACT,1);
		$data['number'] = '95ZYCT'.str_pad($count[numrows]+1,4,"0",STR_PAD_LEFT);
		//end
	...
	private function validform($data) {
		...
		$data['type']          = intval($this->input->get('type',TRUE))==10 ? 10 : -10;
