...
class Home extends CI_Controller {
	...
	public function set_password(){
		...
		if (is_array($data)&&count($data)>0) {
 			//str_alert(-1,$data['newPassword'].$data['buyerMobile'].$data['buyerName']);
 			//exit();
			$info['userpwd'] = md5($data['newPassword']);
			$info['mobile']  = $data['buyerMobile'];
			$info['name']    = $data['buyerName'];
			$this->mysql_model->get_count(STAFF,'(id<>'.$this->jxcsys['uid'].') and mobile='.$info['mobile'].'') >0 && str_alert(-1,'该手机号已被使用,请更换手机号码');
		    $sql = $this->mysql_model->update(STAFF,$info,'(id='.$this->jxcsys['uid'].')');
			if ($sql) {
		    ...
		    $data = $this->mysql_model->get_row(STAFF,'(id='.$this->jxcsys['uid'].')');
