...
<script>
  (function($){
    ...
    $(".grid-wrap").on("click", ".edituer", function(a) {
			a.preventDefault();
			var b = $(this).parents('tr').attr('id');
            window.location.href="../settings/authority_edit?action=edituser&id=" + b;
		});
        $(".grid-wrap").on("click", ".ui-icon-trash", function(a) {
			if (a.preventDefault(), Business.verifyRight("SA_DELETE")) {
				var id = $(this).parents('tr').attr('id');
		      	var rowData = $('#grid').getRowData(id);
		      	var userName = rowData.userName;
				$.dialog.confirm("您确定要删除该管理员吗？", function() {
					Public.ajaxPost("../right/deleteuser?action=deleteuser", {
						userName: userName
					}, function(a) {
						200 === a.status ? 
							($("#grid").jqGrid("delRowData", id),parent.Public.tips({content: "删除成功！"}))
    ...
    function initGrid(){
    function a(a, b, c) {
			var d = '<div class="operating" data-id="' + c.id + '"><a class="edituer ui-icon ui-icon-pencil" title="修改"></a>' +
                '<a class="ui-icon ui-icon-trash" title="删除"></a></div>';
			return d;
		}
      $('#grid').jqGrid({
        ...
        colNames:['操作','用户', '真实姓名', '公司','功能授权','数据授权','启用授权'],
        colModel:[
        {name: "operating",
        index:'operating', 
				width: 60,
