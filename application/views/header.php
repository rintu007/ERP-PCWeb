<?php if(!defined('BASEPATH')) exit('No direct script access allowed');?>
<!doctype html>
<html>
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=1280, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">
<meta name="renderer" content="webkit|ie-stand|ie-comp">
<title>在线进销存</title>
<link href="<?php echo skin_url()?>/css/jquery-ui.min.css" rel="stylesheet" type="text/css">
<link href="<?php echo skin_url()?>/css/common.css?ver=20150522" rel="stylesheet" type="text/css">
<link href="<?php echo skin_url()?>/css/<?php echo sys_skin()?>/ui.min.css?ver=20150522" rel="stylesheet">

<script src="<?php echo skin_url()?>/js/common/libs/jquery/jquery-1.10.2.min.js"></script>
<script src="<?php echo skin_url()?>/js/common/plugins/jquery-ui.min.js"></script>
<script src="<?php echo skin_url()?>/js/common/libs/json3.min.js"></script>
<script src="<?php echo skin_url()?>/js/common/common.js?ver=20170927"></script>
<script src="<?php echo skin_url()?>/js/common/grid.js?ver=20150522"></script>
<script src="<?php echo skin_url()?>/js/common/plugins.js?ver=20150522"></script>
<script src="<?php echo skin_url()?>/js/common/plugins/jquery.dialog.js?self=true&ver=20150522"></script>